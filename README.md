...
**Known limitation — password hashing on Cloudflare:** the Workers runtime caps PBKDF2 at 100,000 iterations, and `ADMIN_PASSWORD_HASH` / customer credentials use 600,000. Verification therefore throws `NotSupportedError: Pbkdf2 failed: iteration counts above 100000 are not supported` and `/api/admin/login`, `/api/account/signup` and `/api/account/login` answer **500** on Pages (reads and every other route are unaffected; Vercel's Node runtime has no such cap). The stored hash format records its own iteration count, so this cannot be fixed by lowering the value in code alone — the credential has to be re-hashed under the new work factor, which is a security decision. Until then, use the Vercel deployment for admin/customer sign-in.
