/**
 * Basco Sports – Salman OS Browser Client
 *
 * The browser NEVER talks to Salman OS directly and NEVER sees credentials.
 * It only calls Basco's own consolidated server proxy (/api/salman-os),
 * which requires an admin session and returns safe payloads only.
 *
 * Contract-gated: when SALMAN_OS_BASE_URL / SALMAN_OS_TOKEN are not
 * configured, the proxy answers state=WAITING and the UI shows
 * "AI BACKEND — WAITING FOR SALMAN OS". All failures degrade to
 * empty/WAITING — commerce never depends on this.
 */

import type {
  SalmanOsStatus,
  SalmanOsIntelligenceItem,
