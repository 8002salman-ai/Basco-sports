...
export default function HomePage() {
  ...
  return (
    <div className="bg-[#FCFBF9]">
      {/* Hero */}
      <section className="relative max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 pt-4 lg:pt-5">
        <div className="relative rounded-[24px] overflow-hidden bg-obsidian text-white min-h-[400px] sm:min-h-[440px] lg:min-h-[470px] xl:min-h-[530px] flex">
          ...
          <div className="relative z-10 w-full flex flex-col lg:flex-row">
            <div className="flex-1 p-6 sm:p-8 lg:p-10 flex flex-col justify-end">
              ...
              <div className="mt-auto lg:mt-4 max-w-[640px]">
                <h1 className="font-display text-[clamp(34px,4.6vw,62px)] leading-[0.95] tracking-[-0.03em] font-bold">
                  Gear built<br />for the long<br /><span className="text-lime">run.</span>
                </h1>
                <p className="mt-3 lg:mt-4 text-[15px] lg:text-[16px] leading-relaxed text-white/70 max-w-[520px]">
                ...
                <div className="mt-5 flex flex-wrap gap-2.5">
                  <Link href="/shop" className="h-11 px-6 rounded-full bg-white text-obsidian font-semibold text-[14px] inline-flex items-center gap-2 hover:bg-stone-100 transition-colors">
                    Shop new arrivals <ArrowUpRight className="w-4 h-4" />
                  </Link>
                  <Link href="/category/running" className="h-11 px-6 rounded-full bg-white/10 backdrop-blur text-white font-medium text-[14px] inline-flex items-center gap-2 hover:bg-white/20 transition-colors border border-white/10">
                    Explore running
                  </Link>
                </div>
                <div className="mt-6 grid grid-cols-3 gap-4 max-w-[440px] border-t border-white/10 pt-4">
                  <div><div className="font-display text-[17px] lg:text-[19px] font-bold">Worldwide</div><div className="text-[10px] lg:text-[11px] tracking-widest uppercase text-white/50">Tracked delivery</div></div>
                  <div><div className="font-display text-[17px] lg:text-[19px] font-bold">30 days</div><div className="text-[10px] lg:text-[11px] tracking-widest uppercase text-white/50">Voluntary returns</div></div>
                  <div><div className="font-display text-[17px] lg:text-[19px] font-bold">Pre-pay</div><div className="text-[10px] lg:text-[11px] tracking-widest uppercase text-white/50">Total shown first</div></div>
                </div>
              </div>
            </div>

            {/* Right card stack */}
            <div className="lg:w-[360px] xl:w-[380px] p-4 lg:p-5 hidden lg:flex flex-col gap-3.5 justify-end">
              <div className="rounded-[20px] bg-white text-obsidian p-5 shadow-lift">
                ...
                <div className="mt-4 flex gap-4">
                  ...
                  <div>
                    ...
                    {products[10].rating > 0 && products[10].reviewCount > 0 && (
        ...
        <div className="mt-4 grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <div key={item.title} className="flex items-center gap-3 rounded-2xl bg-white border border-stone-200/70 px-4 py-3">
      ...
      {/* Shop by sport */}
      <section className="max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 pt-10 lg:pt-14 pb-10 lg:pb-14">
        <div className="flex items-end justify-between gap-6">
          <div>
            <div className="text-[11px] tracking-[0.2em] uppercase opacity-50">Curated discovery</div>
            <h2 className="mt-2 font-display text-[26px] lg:text-[36px] leading-[0.95] tracking-tight">Shop by sport</h2>
          </div>
        ...
        <div className="mt-6 lg:mt-8 grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-5">
          {categories.filter(c => c.slug !== "deals").map(cat => (
            <Link key={cat.slug} href={`/category/${cat.slug}`} className="group relative rounded-[24px] overflow-hidden aspect-[4/5] bg-obsidian">
              ...
              <div className="absolute bottom-0 p-5">
                <div className="text-[10px] tracking-widest uppercase opacity-60 text-white">Collection</div>
                <div className="mt-1 font-display text-[22px] lg:text-[24px] leading-none text-white">{cat.name}</div>
                <div className="mt-2.5 inline-flex items-center gap-2 text-[12px] px-3 py-1.5 rounded-full bg-white text-obsidian font-medium group-hover:bg-lime transition-colors">Shop now <ArrowUpRight className="w-3.5 h-3.5" /></div>
              </div>
            </Link>
          ))}
        </div>
      ...
      {/* Trending */}
      <section className="max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 pb-10 lg:pb-14">
        <div className="flex items-end justify-between">
          <h2 className="font-display text-[24px] lg:text-[32px] leading-none">Trending now</h2>
          <Link href="/shop?sort=trending" className="text-[13px] font-medium underline underline-offset-4">Shop trending</Link>
        </div>
        <div className="mt-5 lg:mt-7 grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-5">
      ...
      <section className="max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 pb-10 lg:pb-14">
        <div className="grid lg:grid-cols-2 gap-3 lg:gap-5">
          <div className="relative rounded-[24px] overflow-hidden min-h-[340px] lg:min-h-[380px] bg-stone-100">
            ...
            <div className="absolute bottom-0 p-6 lg:p-8 text-white max-w-[520px]">
              <span className="px-3 py-1 rounded-full bg-lime text-obsidian text-[11px] font-bold tracking-widest">GYM & TRAINING</span>
              <h3 className="mt-3 font-display text-[26px] lg:text-[34px] leading-[0.95]">Strength without the noise.</h3>
              <p className="mt-2.5 text-white/70 text-[14px] lg:text-[15px]">Bare steel, full-grain leather, no plastic. Gear that earns its place on the rack.</p>
              <Link href="/category/gym" className="mt-5 inline-flex h-11 px-6 rounded-full bg-white text-obsidian font-semibold text-[14px] items-center gap-2">Shop gym <ArrowUpRight className="w-4 h-4" /></Link>
            </div>
          </div>
          <div className="grid gap-3 lg:gap-5">
            <div className="relative rounded-[24px] overflow-hidden min-h-[230px] lg:min-h-[240px] bg-obsidian text-white p-6 lg:p-8 flex flex-col justify-between">
            ...
            <div className="rounded-[24px] bg-lime p-6 lg:p-8 flex flex-col justify-between min-h-[230px] lg:min-h-[240px]">
              <div>
      ...
      <section className="bg-white border-y border-stone-200/70">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 py-10 lg:py-14">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-[24px] lg:text-[32px] leading-none">Featured collections</h2>
      ...
      {/* New arrivals */}
      <section className="max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 py-10 lg:py-14">
        <div className="flex items-end justify-between gap-6">
          <h2 className="font-display text-[24px] lg:text-[32px] leading-none">New arrivals</h2>
      ...
      <section className="max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 pb-6">
      ...
      <section className="bg-obsidian text-white">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-5 lg:px-6 py-6 flex flex-wrap items-center justify-between gap-3 text-[12px] tracking-[0.15em] uppercase opacity-70">
